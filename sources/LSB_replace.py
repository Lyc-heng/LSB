...
def embed(image_path):
    ...
    # 存储结构的数组
    temp_bin = [0 for x in range(code_length*8)]

    # 用来标记读取到第几个数据
    index = 8
    # 将水印信息挨个提取出来
    for c in range(0, code_length*8):
        for n in range(0, 8):
...
def encode(image_path, markedfile, str):
    ...
    for i in range(len(im_array_flatten)):
        ...
        temp = list(temp)
        if temp[-1] != watermark[w_index]:
            temp[-1] = watermark[w_index]
...
if __name__ == '__main__':
    ...
    encode("3-0.bmp", "replace.bmp", str)
    end_CPU = time.clock()
    print("本次程序运行的时间为%.10f秒" % (end_CPU - start_CPU))
    print("嵌入的比特数%d" % (count))
    print(count0)
    print(count1)
    # 将图片水印中的信息提取出来
    # embed("yas.bmp")
